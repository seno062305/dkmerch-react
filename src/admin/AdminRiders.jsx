    ...
    <div className="id-card-overlay" onClick={onClose}>
      <div className="app-detail-modal" style={{ maxWidth: 480 }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ marginBottom: 20 }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontWeight: 600, color: '#374151', marginBottom: 8, fontSize: 13 }}>
            <i className="fas fa-sticky-note" style={{ color: '#6b7280' }}></i> Note <span style={{ fontSize: 11, fontWeight: 400, color: '#9ca3af' }}>(optional)</span>
          </label>
        ...
        {reason && (
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, background: '#fffbeb', border: '1.5px solid #fde68a', borderRadius: 10, padding: '12px 14px', marginBottom: 20, fontSize: 13, color: '#92400e' }}>
            ...
            <span>An email notification will be sent to <strong>{app.email}</strong>.</span>
          </div>
        )}
...
const StatusBadge = ({ order }) => {
  ...
  const map = { confirmed: { label: '✅ Confirmed', bg: '#059669' }, processing: { label: '⚙️ Processing', bg: '#6366f1' }, shipped: { label: '📦 Shipped', bg: '#0ea5e9' }, out_for_delivery: { label: '🚚 Out for Delivery', bg: '#f59e0b' }, completed: { label: '✅ Delivered', bg: '#16a34a' }, delivered: { label: '✅ Delivered', bg: '#16a34a' } };
  const m = map[s];
  return m ? <span className="ar-status-badge" style={{ background: m.bg }}>{m.label}</span> : <span className="ar-status-badge" style={{ background: '#6b7280' }}>{order.orderStatus || order.status}</span>;
};
...
const NotificationBell = () => {
  ...
  const timeAgoStr = (iso) => { const d = Date.now() - new Date(iso).getTime(); const m = Math.floor(d / 60000); if (m < 1) return 'just now'; if (m < 60) return `${m}m ago`; const h = Math.floor(m / 60); if (h < 24) return `${h}h ago`; return `${Math.floor(h / 24)}d ago`; };
  return (
...
/* ─────────────────────────────────────────────────────────────
   Live Map Modal
   FIX 1: auto-center on last known rider location when offline
   FIX 2: burger hidden via AdminLayout MutationObserver
───────────────────────────────────────────────────────────── */
const LiveMapModal = ({ orderId, order, onClose, fullscreen = false }) => {
  ...
  // Track whether we've already auto-centered on last known location
  const centeredOnLastKnownRef = useRef(false);
  ...
  // Lock body scroll + add class for burger hiding
  useEffect(() => {
  ...
  // Init map — center on delivery destination first
  useEffect(() => {
  ...
  // Auto-center on last known location once when map + location data are ready
  useEffect(() => {
    if (!mapObjRef.current || !location || centeredOnLastKnownRef.current) return;
    // If rider is live, don't override — the live tracking useEffect will handle centering
    if (isLive && hasLoc) return;
    // If offline but has last known GPS — center there immediately
    if (!isLive && hasLastKnown) {
      centeredOnLastKnownRef.current = true;
      try {
        mapObjRef.current.setView([location.lastKnownLat, location.lastKnownLng], 15, { animate: true });
      } catch {}
    }
  ...
  const drawRoute = (lat, lng, destLat, destLng, dashed = false) => {
    ...
    routeTimerRef.current = setTimeout(async () => {
      ...
      if (dashed) {
        try { routeLineRef.current = L.polyline([[lat, lng], [destLat, destLng]], { color: '#9ca3af', weight: 3, opacity: 0.75, dashArray: '8, 8' }).addTo(map); map.fitBounds(L.latLngBounds([lat, lng], [destLat, destLng]), { padding: [48, 48], maxZoom: 17 }); } catch {}
        return;
      }
      const controller = new AbortController(); routeAbortRef.current = controller;
      try {
        const res  = await fetch(`https://router.project-osrm.org/route/v1/bike/${lng},${lat};${destLng},${destLat}?overview=full&geometries=geojson`, { signal: controller.signal });
        ...
        if (data.code === 'Ok' && data.routes?.[0]?.geometry?.coordinates) {
          ...
          routeLineRef.current = [
            L.polyline(coords, { color: 'white',   weight: 7, opacity: 0.6 }).addTo(map),
            L.polyline(coords, { color: '#e53e3e', weight: 4.5, opacity: 0.92 }).addTo(map),
          ];
          map.fitBounds(L.latLngBounds(coords), { padding: [48, 48], maxZoom: 17 });
        } else throw new Error('no route');
      } catch (err) {
        if (err?.name === 'AbortError') return;
        try { if (order?.addressLat && order?.addressLng) { routeLineRef.current = window.L.polyline([[lat, lng], [order.addressLat, order.addressLng]], { color: '#e53e3e', weight: 4, opacity: 0.8, dashArray: '10, 8' }).addTo(mapObjRef.current); mapObjRef.current.fitBounds(window.L.latLngBounds([lat, lng], [order.addressLat, order.addressLng]), { padding: [48, 48], maxZoom: 17 }); } } catch {}
  ...
  useEffect(() => {
    ...
    const dLat = order?.addressLat; const dLng = order?.addressLng;
    ...
    if (isLive && hasLoc) {
      ...
      if (!markerRef.current) {
        const icon = L.divIcon({ html: `<div style="width:40px;height:40px;background:linear-gradient(135deg,#fc1268,#e0005a);border-radius:50%;border:3px solid white;box-shadow:0 3px 14px rgba(252,18,104,0.55);display:flex;align-items:center;justify-content:center;font-size:20px;">🛵</div>`, className: '', iconSize: [40, 40], iconAnchor: [20, 20] });
        markerRef.current = L.marker([lat, lng], { icon }).addTo(map).bindPopup(`<b>🛵 ${order.riderInfo?.name || 'Rider'}</b><br>📍 Live location`);
      ...
      if (dLat && dLng) drawRoute(lat, lng, dLat, dLng, false);
    ...
    if (!isLive && hasLastKnown) {
      ...
      if (!lastKnownMarkerRef.current) {
        try { lastKnownMarkerRef.current = L.marker([lastKnownLat, lastKnownLng], { icon: ghostIcon }).addTo(map).bindPopup(`<div style="min-width:160px"><strong>🛵 ${order.riderInfo?.name || 'Rider'}</strong><br><span style="color:#f59e0b;font-weight:600;font-size:12px">⚠️ Last seen ${lastTimeStr}</span>${lastKnownAddress ? `<br><span style="color:#6b7280;font-size:11px">📍 ${lastKnownAddress}</span>` : ''}</div>`); } catch {}
      } else {
        try { lastKnownMarkerRef.current.setLatLng([lastKnownLat, lastKnownLng]); lastKnownMarkerRef.current.setIcon(ghostIcon); } catch {}
      }
      if (dLat && dLng) drawRoute(lastKnownLat, lastKnownLng, dLat, dLng, true);
...
/* ─────────────────────────────────────────────────────────────
   Active Order Card — no OTP for privacy
───────────────────────────────────────────────────────────── */
const ActiveOrderCard = ({ order, collapsed, onToggleCollapse, onOpenMap }) => {
  const { orderId } = order;
  const pickupReq  = order._pickupReq || {};
  const _base      = order.riderInfo  || {};
  const riderInfo  = {
    name:    pickupReq.riderName    || _base.name    || '',
    phone:   pickupReq.riderPhone   || _base.phone   || '',
    plate:   pickupReq.riderPlate   || _base.plate   || '',
          ...
          <div className="ar-section">
            ...
            {hasRiderInfo ? (
              ...
              <div className="ar-rider-waiting-notice">
                ...
                <span>Rider info will appear once a rider accepts this order from the <strong>Rider Dashboard</strong>.</span>
              </div>
            )}
          </div>
...
const CompletedOrderCard = ({ order, onRemove }) => {
  ...
  const deliveredAt = order.deliveryConfirmedAt ? new Date(order.deliveryConfirmedAt).toLocaleString('en-PH', { month: 'short', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : null;
  return (
    <div className="ar-order-card ar-completed-card">
      ...
      <div className="ar-completed-body">
        ...
        {ri.name      && <div className="ar-completed-row"><span className="ar-info-label">Rider</span><span className="ar-info-value">🛵 {ri.name}{ri.plate ? ` · ${ri.plate}` : ''}</span></div>}
...
const AdminRiders = () => {
  ...
  const matchesDateFilter = (o) => {
    ...
    const d = new Date(o._creationTime || o.createdAt || 0);
  ...
  const activeDeliveryItems = allPickupRequests
    ...
    .filter(Boolean).filter(matchesDateFilter)
    .sort((a, b) => (b._creationTime || 0) - (a._creationTime || 0));

  const completedOrders = allOrders
    .filter(o => ['completed', 'delivered'].includes((o.orderStatus || o.status || '').toLowerCase()))
  ...
  const approvePickup = async (requestId, riderName) => {
    ...
    try { await approvePickupMutation({ requestId }); alert(`✅ Pickup approved for ${riderName}!`); } catch (err) { console.error(err); alert('Failed to approve pickup.'); }
  };
  ...
  const DateFilter = () => (
    <div className="ar-date-filter">
      <div className="ar-date-filter-inputs">
        <div className="ar-date-filter-item"><label htmlFor="arStart"><i className="fas fa-calendar"></i></label><input type="date" id="arStart" value={startDate} max={endDate || todayStr} onChange={e => setStartDate(e.target.value)} /></div>
        <span className="ar-date-filter-sep">to</span>
        <div className="ar-date-filter-item"><label htmlFor="arEnd"><i className="fas fa-calendar"></i></label><input type="date" id="arEnd" value={endDate} min={startDate} max={todayStr} onChange={e => setEndDate(e.target.value)} /></div>
  ...
  return (
    <div className="admin-riders">
                ...
                {applications.map(app => (
                  <tr key={app._id}>
                    ...
                    <td className="riders-actions">
                      ...
                      <button className="riders-btn reject" onClick={() => setRejectTarget(app)}>❌ Reject</button>
                    </td>
                  </tr>
                ))}
              </tbody>
              ...
              <tbody>
                {pendingPickups.map(req => (
                  <tr key={req._id}>
                    ...
                    <td>{req.customerName || '—'}</td><td>₱{(req.total || 0).toLocaleString()}</td>
      ...
      {selectedApp   && <ApplicationDetailModal app={selectedApp} onClose={() => setSelectedApp(null)} onApprove={handleApproveRider} onRejectWithReason={r => setRejectTarget(r)} />}
