...
const useTimeAgo = (ts) => {
  ...
  useEffect(() => {
    if (!ts) { setDisplay('unknown'); return; }
    const update = () => {
      const diff = Math.floor((Date.now() - ts) / 1000);
      if (diff < 10)   return setDisplay('just now');
      if (diff < 60)   return setDisplay(`${diff}s ago`);
      if (diff < 3600) return setDisplay(`${Math.floor(diff / 60)}m ago`);
      setDisplay(`${Math.floor(diff / 3600)}h ago`);
    };
    update();
...
const RiderIdCardModal = ({ rider, onClose }) => {
  ...
  const handlePrint = () => {
    ...
    win.document.write(`<html><head><title>DKMerch Rider ID</title><style>body{margin:0;padding:20px;font-family:'Segoe UI',sans-serif;background:#f0f0f0;}.dk-id-card{width:380px;min-height:220px;background:linear-gradient(135deg,#fc1268,#e0005a);border-radius:16px;padding:20px;color:white;box-shadow:0 8px 32px rgba(252,18,104,0.4);position:relative;overflow:hidden;}</style></head><body>${cardRef.current.innerHTML}</body></html>`);
        ...
        <div ref={cardRef}>
          <div className="dk-id-card">
            ...
            <div className="dk-id-footer">
              ...
              <div className="dk-id-valid">Issued: {new Date(rider.dkRiderIdGeneratedAt || Date.now()).toLocaleDateString('en-PH', { year: 'numeric', month: 'short', day: 'numeric' })}</div>
            </div>
          </div>
        </div>
...
const ApplicationDetailModal = ({ app, onClose, onApprove, onRejectWithReason }) => {
  ...
  return (
    <div className="id-card-overlay" onClick={onClose}>
        ...
        <div className="app-detail-photos">
          {[{ key: 'riderPhoto', label: '🤳 Selfie' }, { key: 'validId1', label: '🪪 Valid ID #1' }, { key: 'validId2', label: '🪪 Valid ID #2' }].map(({ key, label }) => (
            <div key={key} className="app-photo-box">
              ...
              {app[key] ? <img src={app[key]} alt={label} className="app-photo-img" onClick={() => setViewImg(app[key])} /> : <div className="app-photo-missing">No photo</div>}
            </div>
          ))}
        </div>
      ...
      {viewImg && <div className="img-viewer-overlay" onClick={() => setViewImg(null)}><img src={viewImg} alt="ID" className="img-viewer-img" /><div className="img-viewer-hint">Tap to close</div></div>}
    </div>
  );
};
    ...
    <div className="id-card-overlay" onClick={onClose}>
      <div className="app-detail-modal" style={{ maxWidth: 480 }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ marginBottom: 16 }}>
          <label style={{ fontWeight: 600, color: '#374151', marginBottom: 8, fontSize: 13, display: 'flex', alignItems: 'center', gap: 6 }}>
            <i className="fas fa-exclamation-triangle" style={{ color: '#dc2626' }}></i> Reason <span style={{ color: '#dc2626' }}>*</span>
          </label>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            {REJECT_REASONS.map(r => (
              <button key={r.value} type="button" onClick={() => setReason(r.value)}
                style={{ padding: '10px 12px', border: `2px solid ${reason === r.value ? 'transparent' : '#e2e8f0'}`, borderRadius: 10, background: reason === r.value ? 'linear-gradient(135deg,#dc2626,#b91c1c)' : 'white', color: reason === r.value ? 'white' : '#4a5568', cursor: 'pointer', textAlign: 'left', display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        <div style={{ marginBottom: 20 }}>
          <textarea style={{ width: '100%', padding: '10px 12px', border: '2px solid #e2e8f0', borderRadius: 10, fontSize: 13, fontFamily: 'inherit', resize: 'vertical', boxSizing: 'border-box', outline: 'none' }} placeholder="Additional note (optional)..." value={note} onChange={e => setNote(e.target.value)} rows={2} />
        </div>
...
const StatusBadge = ({ order }) => {
  ...
  return m
    ...
    : <span className="ar-status-badge" style={{ background: '#6b7280' }}>{s}</span>;
};
...
const LiveBadge = ({ orderId }) => {
  ...
  return <span className="ar-live-indicator"><span className="ar-gps-live-dot" style={{ width: 7, height: 7 }} />Live</span>;
};
...
const NotificationBell = () => {
  ...
  return (
    <div className="ar-bell-wrap" ref={dropdownRef}>
      <button className="ar-bell-btn" onClick={handleOpen}>
...
const LiveMapModal = ({ orderId, order, onClose, fullscreen = false }) => {
  ...
  const lastRoutePlottedRef = useRef({ lat: null, lng: null });
  ...
  useEffect(() => {
    const measure = () => {
      const s = document.querySelector('.admin-sidebar') || document.querySelector('aside');
      if (s) setSidebarWidth(s.getBoundingClientRect().width);
    };
  ...
  useEffect(() => {
    pollIntervalRef.current = setInterval(() => {}, 3000);
  ...
  // Init map — NCR-locked with minZoom + maxBounds + maxBoundsViscosity
  useEffect(() => {
    ...
    // Also call setMaxBounds after init for extra enforcement
    map.setMaxBounds(NCR_BOUNDS);

    const destLat = order?.addressLat;
    const destLng = order?.addressLng;

    const destInNCR = destLat && destLng &&
      destLat >= 14.35 && destLat <= 14.80 &&
      destLng >= 120.86 && destLng <= 121.15;
    ...
    if (destInNCR) {
      ...
      map.setView(NCR_CENTER, NCR_DEFAULT_ZOOM);
    }
  ...
  useEffect(() => {
    if (!mapReadyRef.current || !mapObjRef.current || !location || !window.L) return;
    if (isLive) return;
    ...
    if (prev.lat === lastKnownLat && prev.lng === lastKnownLng && lastKnownMarkerRef.current) return;
    lastPlottedRef.current = { lat: lastKnownLat, lng: lastKnownLng };
    ...
    const ghostIcon = L.divIcon({
      ...
      html: `<div style="position:relative;width:40px;height:40px;">
        <div style="width:40px;height:40px;background:linear-gradient(135deg,#6b7280,#9ca3af);border-radius:50%;border:3px solid white;display:flex;align-items:center;justify-content:center;font-size:20px;opacity:0.88;">🛵</div>
      ...
      try {
        lastKnownMarkerRef.current = L.marker([lastKnownLat, lastKnownLng], { icon: ghostIcon })
          ...
          .bindPopup(`<div style="min-width:180px">
            ...
            <span style="color:#f59e0b;font-weight:600;font-size:12px">⚠️ Last seen ${lastTimeStr}</span>
            ${lastKnownAddress ? `<br><span style="color:#374151;font-size:11px">📍 ${lastKnownAddress}</span>` : ''}
          </div>`)
          .openPopup();
      } catch {}
    } else {
    ...
    try { map.setView([lastKnownLat, lastKnownLng], 15, { animate: true }); } catch {}
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [location, isLive, hasLastKnown, mapReadyRef.current]);

  const clearRoute = useCallback(() => {
    if (routeLineRef.current) {
      const lines = Array.isArray(routeLineRef.current) ? routeLineRef.current : [routeLineRef.current];
      lines.forEach(l => { try { l.remove(); } catch {} });
      routeLineRef.current = null;
    }
    lastRoutePlottedRef.current = { lat: null, lng: null };
  }, []);
    ...
    routeTimerRef.current = setTimeout(async () => {
      ...
      try {
        ...
        if (data.code === 'Ok' && data.routes?.[0]?.geometry?.coordinates) {
          ...
          lastRoutePlottedRef.current = { lat, lng };
          map.fitBounds(L.latLngBounds(coords), { padding: [48, 48], maxZoom: 17 });
        } else { throw new Error('no route'); }
      } catch (err) {
        if (err?.name === 'AbortError') return;
        try {
          if (destLat && destLng && mapObjRef.current) {
            routeLineRef.current = window.L.polyline([[lat, lng], [destLat, destLng]], {
              color: '#e53e3e', weight: 4, opacity: 0.8, dashArray: '10, 8'
            }).addTo(mapObjRef.current);
            mapObjRef.current.fitBounds(window.L.latLngBounds([lat, lng], [destLat, destLng]), { padding: [48, 48], maxZoom: 17 });
          }
        } catch {}
      } finally {
  ...
  // Update rider marker
  useEffect(() => {
    if (!mapObjRef.current || !window.L || !isLive || !hasLoc) return;
    const L = window.L; const map = mapObjRef.current;
    const destLat = order?.addressLat; const destLng = order?.addressLng;
    const { lat, lng, accuracy, updatedAt } = location;

    const prev = lastPlottedRef.current;
    const moved = prev.lat !== lat || prev.lng !== lng;
    if (!moved && markerRef.current) return;
    ...
    if (accuracy) {
      try { circleRef.current = L.circle([lat, lng], { radius: accuracy, color: '#fc1268', fillColor: '#fc1268', fillOpacity: 0.07, weight: 1.5 }).addTo(map); } catch {}
    }

    const lastR = lastRoutePlottedRef.current;
    const movedFarFromRoute = !lastR.lat ||
      Math.abs(lat - lastR.lat) > 0.00015 ||
      Math.abs(lng - lastR.lng) > 0.00015;
    const noRoute = !routeLineRef.current;

    if (destLat && destLng && (noRoute || movedFarFromRoute)) {
      drawRoute(lat, lng, destLat, destLng, false);
    } else if (!destLat || !destLng) {
      try { map.flyTo([lat, lng], Math.max(map.getZoom(), 16), { animate: true, duration: 1 }); } catch {}
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [location, isLive, hasLoc, drawRoute]);
  ...
  const lastKnownAt      = location?.lastKnownAt;
  const lastKnownTimeStr = lastKnownAt ? new Date(lastKnownAt).toLocaleTimeString('en-PH', { hour: '2-digit', minute: '2-digit' }) : null;
  const lastKnownAgoStr  = lastKnownAt ? timeAgo(lastKnownAt) : null;
      ...
      <div className={`ar-map-modal ${isFullscreen ? 'ar-map-modal--full' : ''}`}
        ...
        <div className="ar-map-header" style={{ flexShrink: 0 }}>
          <div className="ar-map-title-row">
            ...
            <span className="ar-ncr-label">NCR Only</span>
            {isDelivered && <span style={{ background: '#d1fae5', color: '#065f46', borderRadius: 20, padding: '3px 10px', fontSize: 11, fontWeight: 700 }}>✅ Delivered</span>}
          </div>
          <div className="ar-map-header-right">
            {isLive
              ? <div className="ar-map-live-badge">
                  <span className="ar-gps-live-dot" />
                  Live · {location?.updatedAt
                    ? new Date(location.updatedAt).toLocaleTimeString('en-PH', { hour: '2-digit', minute: '2-digit', second: '2-digit' })
              ...
              : hasLastKnown
                ...
                : !location
                  ? <div className="ar-map-waiting-badge">⏳ Waiting for rider GPS…</div>
                  : <div className="ar-map-stale-badge">⏸ {new Date().toLocaleTimeString('en-PH', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</div>
            }
        ...
        {!location && (
          <div className="ar-map-waiting-overlay" style={{ flexShrink: 0 }}>
            ...
            <span>Waiting for rider GPS… Map will update automatically when rider starts tracking.</span>
          </div>
        )}
        ...
        {!isLive && hasLastKnown && (
          <div className="ar-map-offline-bar">
            ...
            <span>
              <strong>Rider offline</strong> · Last known {lastKnownTimeStr} ({lastKnownAgoStr})
              {location?.lastKnownAddress && <> · <strong>📍 {location.lastKnownAddress}</strong></>}
            </span>
          </div>
        )}
...
// Active Order Card
// ─────────────────────────────────────────────────────────────
const ActiveOrderCard = ({ order, collapsed, onToggleCollapse, onOpenMap }) => {
  const { orderId } = order;
  ...
  const alreadyNotified = (order.orderStatus || '').toLowerCase() === 'out_for_delivery' || pickupReq.status === 'out_for_delivery';
  ...
  return (
    <div className="ar-order-card ar-completed-card">
      <div className="ar-order-header ar-completed-header">
        ...
        <button className="ar-remove-btn" onClick={() => onRemove(order)}>🗑️</button>
      </div>
...
const AdminRiders = () => {
  ...
  const handleApproveRider = async (app) => {
    if (!window.confirm(`Approve "${app.fullName}" as a rider?\n\nEmail: ${app.email}\nVehicle: ${app.vehicleType || 'N/A'}`)) return;
    try {
      const result = await approveRiderMutation({ id: app._id });
      try { await sendRiderApprovedEmailAction({ to: app.email, riderName: app.fullName, dkRiderId: result.dkRiderId, email: app.email, vehicleType: app.vehicleType, plateNumber: app.plateNumber }); } catch(e) { console.error(e); }
  ...
  const handleConfirmReject = async (id, name, reason, note) => {
    try {
      ...
      if (app?.email) { try { await sendRiderRejectedEmailAction({ to: app.email, riderName: app.fullName, reason, note: note || undefined }); } catch(e) { console.error(e); } }
      setRejectTarget(null);
    } catch (err) { console.error(err); alert('Failed to reject rider.'); }
  };

  const handleRevokeRider = async (id, name) => {
    if (!window.confirm(`Revoke approval for "${name}"?`)) return;
  ...
  const getPickupStatusColor = (s) => ({ pending: '#ffc107', approved: '#28a745', rejected: '#dc3545', out_for_delivery: '#6366f1', completed: '#059669' }[s] || '#6c757d');
  const getPickupStatusLabel = (s) => ({ pending: '⏳ Pending', approved: '✅ Approved', rejected: '❌ Rejected', out_for_delivery: '🚚 Out for Delivery', completed: '🎉 Delivered' }[s] || s);
  ...
  const DateFilter = () => (
    <div className="ar-date-filter">
      <div className="ar-date-filter-inputs">
        ...
        {(startDate || endDate) && <button className="ar-date-filter-clear" onClick={clearDateFilter}><i className="fas fa-times"></i> Clear</button>}
      </div>
    </div>
  );
      ...
      <div className="riders-tabs">
        {tabs.map(t => (
          <button key={t.key} className={`riders-tab-btn ${tab === t.key ? 'active' : ''}`} onClick={() => setTab(t.key)}>
            {t.label}
            {t.count > 0 && <span className={`riders-tab-count-badge ${t.green ? 'riders-tab-count-badge--green' : ''}`}>{t.count}</span>}
          </button>
        ))}
              ...
              <tbody>
                {applications.map(app => (
                  <tr key={app._id}>
                    ...
                    <td><div className="riders-photo-indicators"><span className={`photo-dot ${app.riderPhoto ? 'has-photo' : 'no-photo'}`}>🤳</span><span className={`photo-dot ${app.validId1 ? 'has-photo' : 'no-photo'}`}>🪪</span><span className={`photo-dot ${app.validId2 ? 'has-photo' : 'no-photo'}`}>🪪</span></div></td>
                  ...
                  <tr key={rider._id}>
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        {rider.riderPhoto
                          ? <img src={rider.riderPhoto} alt={rider.fullName} style={{ width: 32, height: 32, borderRadius: '50%', objectFit: 'cover', border: '2px solid #e2e8f0', flexShrink: 0 }} />
                          : <div style={{ width: 32, height: 32, borderRadius: '50%', background: '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, flexShrink: 0 }}>👤</div>}
                        {rider.fullName}
                      </div>
                    </td>
      ...
      {tab === 'delivery' && (
        <>
          ...
          {activeDeliveryItems.length === 0
            ? <div className="ar-empty"><div className="ar-empty-icon">📭</div><p>No active deliveries{(startDate || endDate) ? ' for selected dates' : ''}.</p><span>Approved pickup orders appear here.</span></div>
            : <div className="ar-orders-list">{activeDeliveryItems.map(o => <ActiveOrderCard key={o._id} order={o} collapsed={!!collapsed[o.orderId]} onToggleCollapse={handleToggleCollapse} onOpenMap={openMap} />)}</div>
          }
        </>
      )}
