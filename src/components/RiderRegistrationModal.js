...
const RiderRegistrationModal = ({ onClose }) => {
  ...
  const validateRiderPassword = (password) => {
    ...
    if (!password.startsWith('@rider')) {
      errs.push('Password must start with @rider.');
    }
    if (password.length > 10) {
      errs.push('Password must be maximum 10 characters.');
    }
    if (password.length < 7) {
      errs.push('Password must be at least 7 characters (e.g. @rider1).');
    }
    return errs;
  };
  ...
  const handleChange = (e) => {
    ...
    if (name === 'password') {
      ...
      setForm({ ...form, password: truncated });
  ...
  const validate = () => {
    ...
    } else if (form.plateNumber.trim().length < 5) {
      newErrors.plateNumber = 'Enter a valid plate number (e.g. ABC 1234).';
    }

    if (!form.licenseNumber.trim()) {
      newErrors.licenseNumber = "Driver's license number is required.";
    } else if (form.licenseNumber.trim().length < 6) {
      newErrors.licenseNumber = 'License number seems too short.';
    }

    const passwordErrs = validateRiderPassword(form.password);
    if (passwordErrs.length > 0) {
      newErrors.password = passwordErrs[0];
    }
  ...
  const handleSubmit = (e) => {
    ...
    const existing = JSON.parse(localStorage.getItem('dkmerch_rider_applications')) || [];
    const duplicateEmail = existing.find(r => r.email === form.email);
    if (duplicateEmail) {
      setErrors({ email: 'This email is already registered as a rider applicant.' });
      return;
    }
          ...
          <form onSubmit={handleSubmit} className="rider-form" noValidate>
            ...
            <div className="rider-form-row">
              <div className="rider-form-group">
                ...
                {errors.vehicleType && (
                  <span className="error-msg">⚠ {errors.vehicleType}</span>
                )}
              </div>
              ...
              <div className="rider-form-group">
                ...
                <div className="field-footer">
                  {errors.plateNumber
                    ? <span className="error-msg">⚠ {errors.plateNumber}</span>
                    : <span />}
            ...
            <div className="rider-form-group">
              ...
              <div className="field-footer">
                {errors.licenseNumber
                  ? <span className="error-msg">⚠ {errors.licenseNumber}</span>
                  : <span />}
            ...
            <button type="submit" className="rider-submit-btn">
              Submit Application
            </button>
