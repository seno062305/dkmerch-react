...
const Header = ({ cartCount, wishlistCount, onCartClick }) => {
  ...
  useEffect(() => {
    ...
    if (location.pathname === '/' && scrollTarget && scrollTarget !== lastScrollTarget.current) {
    ...
    if (location.pathname !== '/') lastScrollTarget.current = null;
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [location.pathname]);

  const handleLogout = () => {
    if (window.confirm('Are you sure you want to logout?')) {
      logout(); setShowAccountDropdown(false); navigate('/');
    }
  };
  ...
  const handleRiderLoginSuccess = (role) => {
    setShowRiderModal(false);
    if (role === 'rider') navigate('/rider', { replace: true });
  };
          ...
          <div className="header-inner">
            ...
            <div className="header-actions">
              ...
              <button className="header-action-item" onClick={() => setShowSearch(true)}>
                <span className="icon-wrapper"><i className="fas fa-search"></i></span>
                <span>Search</span>
              </button>
              ...
              {isAuthenticated ? (
                <div className="account-dropdown-wrapper" ref={dropdownRef}>
                  <button className="account-btn" onClick={() => setShowAccountDropdown(!showAccountDropdown)}>
                    ...
                    {hasAvailable && <span className="preorder-dot-indicator"></span>}
                ...
                <button className="header-action-item" onClick={handleLoginClick}>
                  <span className="icon-wrapper"><i className="fas fa-user"></i></span>
                  <span>Login</span>
                </button>
              )}
          ...
          <div className="search-overlay" onClick={() => setShowSearch(false)}>
            <div className="search-container" onClick={(e) => e.stopPropagation()}>
              <form onSubmit={handleSearchSubmit} className="search-bar-expanded">
                ...
                <input type="text" placeholder="Search for K-Pop merchandise..."
                  value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                  onKeyDown={handleKeyDown} autoFocus />
                {searchQuery && (
                  <button type="button" className="search-clear-input"
